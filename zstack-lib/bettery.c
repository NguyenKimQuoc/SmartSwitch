...
uint16 getBatteryVoltage(void) {
    ...
    zclBattery_RawAdc = adcReadSampled(HAL_ADC_CHN_AIN0, HAL_ADC_RESOLUTION_14, HAL_ADC_REF_125V, 10);
...
//#if BDB_REPORTING
////    LREP("BDB\r\n");
////    bdb_RepChangedAttrValue(1, POWER_CFG, ATTRID_POWER_CFG_BATTERY_VOLTAGE);
//    bdb_RepChangedAttrValue(1, POWER_CFG, ATTRID_POWER_CFG_BATTERY_PERCENTAGE_REMAINING);
////    bdb_RepChangedAttrValue(1, POWER_CFG, ATTRID_POWER_CFG_BATTERY_VOLTAGE_RAW_ADC);
//#else
...
uint8 getBattPercentValue(){
  uint16 millivolts = getBatteryVoltage();
  zclBattery_Voltage = getBatteryVoltageZCL(millivolts);
  zclBattery_PercentageRemainig = ZCL_BATTERY_REPORT_REPORT_CONVERTER(millivolts);
  return zclBattery_PercentageRemainig;
}
...
//  for(i = 0; i<len; i++){
//    sprintf(bufferStr,"data: %d", buf[i]);
//    UART_String(bufferStr);
//  }
  return 0;
}
...
void HalI2CSendCommand(uint8 address, uint8 cmd){
  hali2cSendDeviceAddress(address);
  hali2cSend((uint8 []){0x00, cmd}, 2, NOSEND_START, SEND_STOP);
}

void HalI2CStartCondition()
{
  hali2cStart();
}
...
bool HalI2CSendByte( uint8 dByte )
{
  if (!hali2cSendByte(dByte ))
    return 0;
  else
    return 1;
}
void HalI2CSendByteLoop(uint8 dByte)
{
  hali2cSend((uint8 []){dByte}, 1, NOSEND_START, NOSEND_STOP);
}
...
void HalI2CRead(uint8 *buf, uint16 len)
{
  uint16 i;
  for ( i = 0; i < len-1; i++ )
  {
